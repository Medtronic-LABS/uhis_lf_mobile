...
class UnifiedFormNotifier extends ChangeNotifier {
  ...
  Future<void> loadPregnancyData() async {
    try {
      ...
      // Seed form-data fields so InfoLabel tiles populate on first render.
      if (lmp != null) {
  ...
  /// Called when `lmp` field changes. Updates EDD + gestationalWeek in
  /// [_data] so InfoLabel tiles refresh immediately, syncs notifier pregnancy
  /// state, and persists to [PregnancySnapshotDao] asynchronously.
