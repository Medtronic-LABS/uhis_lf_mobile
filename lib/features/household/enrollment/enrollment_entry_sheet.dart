...
class _SearchScannerOverlayState extends State<_SearchScannerOverlay>
  ...
  Future<void> _initCamera() async {
    ...
    try {
      ...
      _autoScanTimer = Timer.periodic(const Duration(milliseconds: 1800), (_) {
        if (mounted && !_isScanning && _cameraReady &&
            _overlayState == _OverlayState.scanner) {
          _handleCapture();
        }
      });
  ...
  Future<void> _handleCapture() async {
    ...
    try {
      ...
      if (qr != null) {
        _autoScanTimer?.cancel();
        setState(() {
          _scanned = NidCardData(nidNumber: qr);
          _overlayState = _OverlayState.postScan;
        });
        return;
      }
      ...
      if (!mounted) return;
      if (nid.status == NidScanStatus.success) {
        _autoScanTimer?.cancel();
        setState(() {
          _scanned = nid.data;
          _overlayState = _OverlayState.postScan;
      ...
      // Neither found — suppress snack during auto-scan; show on manual tap
      if (!(_autoScanTimer?.isActive ?? false)) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
        ...
        child: SafeArea(
          child: Stack(
            children: [
              ...
              if (_overlayState == _OverlayState.postScan)
                _SearchPostScanSheet(
                  data: _scanned,
                  onConfirm: () =>
                      Navigator.of(context).pop(_scanned?.nidNumber),
                  onScanAgain: () {
                    setState(() {
                      _scanned = null;
                      _overlayState = _OverlayState.scanner;
...
/// Post-scan confirmation sheet for search mode.
/// Shows the scanned card details; "Search" pre-fills the search field,
/// "Scan again" returns to the viewfinder.
class _SearchPostScanSheet extends StatelessWidget {
  const _SearchPostScanSheet({
    required this.data,
    ...
    return Positioned(
      ...
      child: Container(
        ...
        padding: const EdgeInsets.fromLTRB(
          AppSpacing.h5xl, AppSpacing.xxxl, AppSpacing.h5xl, AppSpacing.h8xl,
        ),
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  _NidField(label: 'NAME', value: name ?? 'Not read', dim: name == null),
                  const _NidDivider(),
                  _NidField(label: 'DATE OF BIRTH', value: dob ?? 'Not read', dim: dob == null),
                  const _NidDivider(),
                  _NidField(label: 'NID NUMBER', value: nid ?? '—', emphasise: true),
                ],
              ),
            ),
            ...
            _SheetOptionButton(
              ...
              title: 'Search for this patient',
              subtitle: 'Pre-fill search bar with this NID',
            ...
            _SheetOptionButton(
              ...
              title: 'Scan again',
              subtitle: 'Point at a different card',
